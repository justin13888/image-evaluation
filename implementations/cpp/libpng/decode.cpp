...
class LibPngBench : public BenchmarkImplementation {
 ...
 private:
  std::vector<uint8_t> decode(const std::vector<uint8_t> &data) {
    ...
    // Expand palette (indexed) images to RGB. Encoders under test may
    // legitimately emit a palette PNG (e.g. zune-png/zenpng picks an indexed
    // colortype when the image has <=256 colours); without this libpng returns
    // 1 byte/pixel (the palette index) and encode_ppm_rgb8() sees a third of
    // the RGB bytes it expects.
