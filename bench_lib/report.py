...
def _quality_section(qual_dir: str) -> list[str]:
    ...
    parts.append(
        ...
        "measurements embedded below. The controls box sets the X axis shared by "
        "every rate-distortion chart — quality vs size (bpp), vs encode time, or "
        "vs decode time — and toggles a logarithmic or linear scale (Alt+[ / "
        "Alt+] cycle the X axis). Each chart overlays every selected format's "
        "encoders, one full-width chart per metric (SSIMULACRA2, PSNR, SSIM, "
        "Butteraugli). Use the <em>filter bar</em> to choose formats, "
        "implementations and metrics across every chart; hover a point for "
    ...
    parts.append(
        ...
        "<h3>Rate-distortion</h3>"
        "<p class='q-note'>One full-width chart per metric, each overlaying every "
        "selected format's encoders on the X axis chosen in the controls box. "
        "Every encoder's quality sweep is aggregated to a mean curve over the "
        "images (see the aggregation note above); each metric's y-axis is fixed to "
        "its known range, so formats are directly comparable. Up and to the "
        "left/up is better.</p>"
        "<div id='q-rd'></div>"
